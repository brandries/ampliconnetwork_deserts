...
tax <- read.delim("./Combined ITS-16S MENA/Control_Aus_zeros_top250.txt", row.names = 1)
mapping_file <- read.delim("./16S/metadata.txt", row.names = 1)
taxonomy_mapping <- read.delim("./16S/rarefaction/networks/FC16S_rep_numbered_tax_assignments_phyloseq_all.txt", row.names = 1)
...
OTU <- otu_table(tax, taxa_are_rows = T)
...
#network
spiec.out=spiec.easi(physeq, method="mb", lambda.min.ratio = 1e-2, nlambda = 9,icov.select.params=list(rep.num=20))
...
#Export to cytoscape
write.graph(spiec.graph,file="spieceasi.ncol_250aus.txt",format="ncol") 
write.table(TAX,file="taxonomy_network_a.txt",sep="\t", quote=FALSE)
